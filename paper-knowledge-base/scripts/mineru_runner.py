"""MinerU subprocess 提取器。

通过 subprocess 调用 MinerU venv 中的 Python 解释器运行 `mineru_extract.py`，
提取 PDF / Word 文档文本。模块级配置（MinerU 目录与解释器路径）由调用方通过
`configure()` 设置。
"""
...
# 模块级配置（由 sync_zotero.main() 通过 configure() 设置）
MINERU_DIR: Path | None = None
MINERU_PYTHON: Path | None = None


def configure(mineru_dir: Path | None, mineru_python: Path | None) -> None:
    """配置 MinerU 目录与 Python 解释器路径。"""
